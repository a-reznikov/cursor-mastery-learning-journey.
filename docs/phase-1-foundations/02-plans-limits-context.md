# Step 2: Plans, Limits & Context Windows

**Phase:** Foundations  
**Date Started:** ____  
**Date Completed:** ____  
**Time Spent:** ___ hours

---

## 📋 Overview

Understanding Cursor's pricing plans, usage limits, token budgets, and how the context window works. This knowledge is essential for team planning and budgeting.

---

## 🎯 Learning Objectives

By the end of this step, I should be able to:

- [ ] Compare Free, Pro, and Business plans
- [ ] Understand token budgets and quotas
- [ ] Explain what 1M token context window means in practice
- [ ] Plan budget for team adoption
- [ ] Implement strategies to manage rate limits

---

## 📚 Topics Covered

### Topic 1: Cursor Pricing Plans

#### Free Plan

**Features:**
- 
- 
- 

**Limitations:**
- 
- 

**Good For:**
- 

**My Notes:**


---

#### Pro Plan ($20/month)

**Features:**
- 500 fast premium requests/month
- Unlimited slow requests
- 
- 

**Limitations:**
- 

**Good For:**
- 

**My Notes:**


---

#### Business Plan (Custom Pricing)

**Features:**
- Everything in Pro
- Centralized billing
- Admin dashboard
- 
- 

**Additional Benefits:**
- 
- 

**Good For:**
- 

**My Notes:**


---

### Topic 2: Token Budgets & Quotas

**What is a Token?**


**Understanding Request Types:**

**Fast Requests:**
- 
- Pro plan: 500/month
- 

**Slow Requests:**
- 
- Pro plan: Unlimited
- 

**How They're Counted:**


**My Notes:**


---

### Topic 3: Context Window (1M Tokens)

**What Does 1M Tokens Mean?**

- In words: ~750,000 words
- In pages: ~3,000 pages
- In code: 

**Practical Examples:**
- Average function: ~___ tokens
- Medium file (200 lines): ~___ tokens
- Chat conversation (20 messages): ~___ tokens

**What Consumes Context:**
- 
- 
- 
- 

**Context Reset:**


**My Notes:**


---

### Topic 4: Rate Limits & Management

**Rate Limits:**


**Strategies to Manage Limits:**

1. **Optimize Context**
   - 
   - 

2. **Strategic Model Selection**
   - 
   - 

3. **Use .cursorignore**
   - 
   - 

**My Notes:**


---

### Topic 5: Cost Analysis for Our Team

**Team Size:** ___ developers

**Estimated Monthly Cost:**

| Plan | Per User | Total Team | Notes |
|------|----------|------------|-------|
| Free | $0 | $0 | Limited, evaluation only |
| Pro | $20 | $____ | Per developer |
| Business | $____ | $____ | Contact sales |

**Our Recommendation:**


**Budget Justification:**
- Current tool costs we can replace: 
- Expected productivity gains: 
- ROI calculation: 

**My Notes:**


---

## 💡 Key Takeaways

1. **[Plan Selection]**
   - 
   - 

2. **[Token Management]**
   - 
   - 

3. **[Team Budget]**
   - 
   - 

---

## 🔍 Practical Examples

### Example 1: Monitoring My Token Usage

**What I Did:**
```
[Steps to check token usage]
```

**What I Found:**
- Average tokens per chat session: 
- Most expensive operation: 
- Tips to reduce usage: 

**Learning:**


---

### Example 2: Testing Rate Limits

**Scenario:**


**What Happened:**


**How I Managed It:**


**Learning:**


---

## ✅ What Worked Well

- 
- 
- 

---

## ⚠️ Challenges & Solutions

### Challenge 1: [e.g., "Understanding token consumption"]

**Problem:**


**Solution:**


**Prevention:**


---

## ❓ Questions & Answers

### Q1: How do I check my current usage?
**A:** 

**Source:** 

---

### Q2: What happens when I hit the limit?
**A:** 

**Source:** 

---

### Q3: Can we share a team account?
**A:** 

**Source:** 

---

### Q4: How do unused requests roll over?
**A:** 

**Source:** 

---

## 🔗 Resources

### Official Documentation
- Cursor Pricing: https://cursor.sh/pricing
- 

### Internal Resources
- Budget proposal template: 
- Team decision doc: 

---

## 📦 Deliverables Created

- [ ] Plan comparison table → Path: `[to be added to team docs]`
- [ ] Team budget proposal → Path: `[budget proposal doc]`
- [ ] Usage monitoring guide → Path: `[team guide]`

---

## 🎬 Next Steps

After completing this step:

1. [ ] Discuss plan selection with team lead
2. [ ] Get budget approval if needed
3. [ ] Set up usage monitoring for team
4. [ ] Move to Step 3: Core Settings & Configuration

---

## 📊 Self-Assessment

Rate your understanding (1-5 scale):

- [ ] 1 - Need to review again
- [ ] 2 - Basic understanding
- [ ] 3 - Good understanding
- [ ] 4 - Strong understanding
- [ ] 5 - Expert level, can teach others

**Confidence Level:** ___/5

**Notes on what to review:**
- 

---

## 💭 Personal Notes & Insights

[Your free-form notes about plans, costs, team needs, etc.]




---

## 🔄 Review Checklist

Before moving to the next step:

- [ ] All learning objectives completed
- [ ] Plan comparison documented
- [ ] Token usage understood
- [ ] Budget calculated for team
- [ ] Rate limit strategies identified
- [ ] Questions answered
- [ ] Key takeaways summarized

---

**Status:** 🟡 In Progress

**Last Updated:** ____

